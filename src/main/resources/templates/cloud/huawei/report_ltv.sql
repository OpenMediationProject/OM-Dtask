INSERT INTO report_ltv
SELECT NULL                                                                                                                   AS id,
       '[(${executeYear})]-[(${executeMonth})]-[(${executeDay})]'                                                             AS execute_date,
       CONCAT(SUBSTR(user_info_data.ymd, 1, 4), '-', SUBSTR(user_info_data.ymd, 5, 2), '-', SUBSTR(user_info_data.ymd, 7, 2)) AS base_date,
       DATEDIFF(
                CONCAT(SUBSTR(user_info_data.ymd, 1, 4), '-', SUBSTR(user_info_data.ymd, 5, 2), '-', SUBSTR(user_info_data.ymd, 7, 2))
           ,
                '[(${executeYear})]-[(${executeMonth})]-[(${executeDay})]'
           )                                                                                                                  AS retention_day,
       ...
       SUM(IF(user_info_data.is_new = 1, NVL(mediation_value_data.mediation_value, 0), 0))                                    AS mediation_value_new,
       SUM(IF(user_info_data.is_new = 0, NVL(mediation_value_data.mediation_value, 0), 0))                                    AS mediation_value_old,
       SUM(IF(user_info_data.is_new = 1, NVL(iap_value_data.iap_value, 0), 0))                                                AS iap_value_new,
       SUM(IF(user_info_data.is_new = 0, NVL(iap_value_data.iap_value, 0), 0))                                                AS iap_value_old,
       SUM(IF(user_info_data.is_new = 1, NVL(mediation_value_data.mediation_value, 0) + NVL(iap_value_data.iap_value, 0), 0)) AS total_value_new,
       SUM(IF(user_info_data.is_new = 0, NVL(mediation_value_data.mediation_value, 0) + NVL(iap_value_data.iap_value, 0), 0)) AS total_value_old,
       NULL                                                                                                                   AS create_time
     ...
     (
         ...
         FROM (
                  SELECT user_behavior_data.country                                                                                                            AS country,
                         ...
                         uid,
                         IF(NVL(cost, 0) = 0 OR impr_total + click_total = 0, 0, ((impr_cur + 4 * click_cur) * NVL(cost, 0)) / (impr_total + 4 * click_total)) AS mediation_value
                  FROM (
                           SELECT country,
                                  ...
                                  iid                                                                                                           AS instance_id,
                                  uid,
                                  SUM(IF(type = 6 OR impr > 0, NVL(impr, 1), 0)) OVER (PARTITION BY country, publisherId, pubAppId, mid, iid)   AS impr_total,
                                   SUM(IF(type = 7 OR click > 0, NVL(click, 1), 0)) OVER (PARTITION BY country, publisherId, pubAppId, mid, iid) AS click_total,
                                   IF(type = 6 OR impr > 0, NVL(impr, 1), 0)                                                                     AS impr_cur,
                                  IF(type = 7 OR click > 0, NVL(click, 1), 0)                                                                   AS click_cur
                           FROM lr
                           WHERE y = '[(${executeYear})]'
                             AND m = '[(${executeMonth})]'
                             AND d = '[(${executeDay})]'
                       ...
                       (
                           ...
                           WHERE day = '[(${executeYear})]-[(${executeMonth})]-[(${executeDay})]'
