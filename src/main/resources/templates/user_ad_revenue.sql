...
FROM
    (
        SELECT
               ...
               sum(impr)                                                                                                                                        AS impr_cnt,
               sum(click)                                                                                                                                       AS click_cnt,
               sum(IF(coalesce(cost, 0) = 0 OR impr_total + click_total = 0, 0, ((impr + 4 * click) * coalesce(cost, 0)) / (impr_total + 4 * click_total)))     AS revenue
        ...
        FROM (
                 SELECT coalesce(pubAppId, 0)                                                                                                                   AS pub_app_id,
                        ...
                        SUM(sum(if(type = 6, 1, 0))) OVER (PARTITION BY coalesce(pubAppId, 0), coalesce(plat, -1), coalesce(mid, 0), country, coalesce(iid, 0)) AS impr_total,
                        SUM(sum(if(type = 7, 1, 0))) OVER (PARTITION BY coalesce(pubAppId, 0), coalesce(plat, -1), coalesce(mid, 0), country, coalesce(iid, 0)) AS click_total
                 FROM lr
                 WHERE
                    ts IS NOT NULL
                    AND y='[(${year})]'
