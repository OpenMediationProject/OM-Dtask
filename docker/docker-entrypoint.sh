...
do
    ...
    if [[ $env_var =~ ^OMDTASK_ ]]; then
        ...
        if [[ ${item_name} = "mountpath" ]]; then
            ...
            loginfo_note "[Cloud Storage] Link ${!env_var}/${CONFFILE}/cache to /${CONFFILE}/cache"
            if [[ -d /${CONFFILE}/cache ]];then
                rm -fr /${CONFFILE}/cache
            fi
            ln -sf ${!env_var}/${CONFFILE}/cache  /${CONFFILE}/cache
    ...
    if [[ $env_var =~ ^OMRSYNC_ ]]; then
        item_name=$(echo "$env_var" | cut -d_ -f2- | tr '[:upper:]' '[:lower:]' | tr _ -)
        if [[ ${item_name} = "module" ]]; then
            loginfo_note "[Configuring] ${item_name} in /etc/rsyncd.conf"
            sed -i "s@om_cache@${!env_var}@g" /etc/rsyncd.conf
            continue
        fi 
        if [[ ${item_name} = "path" ]]; then
            loginfo_note "[Configuring] ${item_name} in /etc/rsyncd.conf"
            sed -i "s@^path.*@path=${!env_var}@g" /etc/rsyncd.conf
            continue
        fi 
        if [[ ${item_name} = "allowhosts" ]]; then
            loginfo_note "[Configuring] ${item_name} in /etc/rsyncd.conf"
            sed -i "s@hosts allow.*@hosts allow=${!env_var}@g" /etc/rsyncd.conf
            continue
        fi 
    fi
